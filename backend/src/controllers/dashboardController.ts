...
export const getDashboardStats = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const totalRequests = await prisma.returnRequest.count();

    const statusCountsGroup = await prisma.returnRequest.groupBy({
      by: ['status'],
      _count: { id: true },
    });
    ...
    const todayRequests = await prisma.returnRequest.count({
      where: {
        createdAt: {
          gte: startOfToday,
        },
    ...
    const issueCountsGroup = await prisma.returnRequest.groupBy({
      by: ['issueType'],
      _count: { id: true },
    });
    ...
    const resolutionCounts = await prisma.returnRequest.groupBy({
    ...
    const recentRequests = await prisma.returnRequest.findMany({
      where: {
        createdAt: {
          gte: sevenDaysAgo,
        },
